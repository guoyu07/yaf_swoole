	...
	*/
	public function connect()
	{
		if (!is_null($this->pdo))
			return $this->pdo;
		try {
			$this->pdo = new PDO($this->conf['dsn'], $this->conf['un'], $this->conf['pw'], array(PDO::MYSQL_ATTR_INIT_COMMAND => $this->conf['charset']));
	...
	*/
	public function query($sql, $data = array(), $one = false, $cache_type = null, $timeout = null)
	{
		...
		if (is_null($cache_type))
			$cache_type = $_conf['cache']['type'];

		if (is_null($timeout))
			$timeout = $_conf['cache']['expire'];
		...
		{
			throw new Exception('error in execute sql');
		}
	...
	*/
	private function __construct($conf)
	{
		if (!extension_loaded('pdo') || !extension_loaded('pdo_mysql'))
			throw new Exception("pdo or mysql pdo extension not find");

		$_conf = Yaf_Registry::get('config');

		$this->conf = array(
			'dsn' => $_conf['resources']['database']['params']['dsn'],
			'un' => $_conf['resources']['database']['params']['username'],
			'pw' => $_conf['resources']['database']['params']['password'],
			'charset' => $_conf['resources']['database']['params']['charset']
		);
